...
{
	assert (params.heap_alloc_object);
	assert (params.heap_alloc_array);
	assert (params.arraylength_get);
...
void dmemory_lower_arraylength(ir_node *call)
{
	...
	ir_node  *len       = (*dmp.arraylength_get)(array_ref, irg, block, &cur_mem);
	ir_node  *in[]      = { len };
