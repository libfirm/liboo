...
DEPS_RT = $(addprefix $(RUNTIME_BUILDDIR)/, $(addsuffix .d, $(basename $(SOURCES_RT))))
...
UNUSED := $(shell mkdir -p $(BUILDDIR)/src-cpp/adt $(RUNTIME_BUILDDIR)/shared/src-cpp/rt $(RUNTIME_BUILDDIR)/static/src-cpp/rt)
...
$(RUNTIME_BUILDDIR)/shared/%.o: %.c
	@echo '===> CC $@'
	$(Q)$(CC) $(CPPFLAGS) $(CFLAGS) $(PIC_FLAGS) -MD -MF $(addprefix $(RUNTIME_BUILDDIR)/, $(addsuffix .d, $(basename $<))) -c -o $@ $<
